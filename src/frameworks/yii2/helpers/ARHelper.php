...
namespace pribolshoy\repository\frameworks\yii2\helpers;
 ...
 *
 * @package pribolshoy\repository\frameworks\yii2\helpers
 */
trait ARHelper
{
    protected $entity;
            ...
            if (method_exists($this->entity,'getOrdersBy')) {
                ...
                if ($orders = $this->entity->getOrdersBy()) {
                    ...
                    foreach ($orders as $order) {
                        $result[] = $this->getModel()::tableName() . '.' . $order;
                    }
                }
                ...
                foreach ($this->entity->attributes() as $attribute) {
                    $result[] = $attribute;
                    $result[] = $this->getModel()::tableName() . '.' . $attribute;
                }
            }
    ...
    public function collectSortingWithDelimiter(string $sorting, string $delimiter = '-', bool $append = true)
    {
        ...
        if (count($sort_parts) === 2) {
            ...
            if (!$this->getAvailableOrdersBy()
                ...
                $orderBy = $this->getModel()::tableName() . '.' . $sort_parts[0];
    ...
    public function getOrderbyByFilter(bool $clear_columns = false)
    {
        ...
        if (is_array($orderBy)) {
            ...
            foreach ($orderBy as $key => $order) {
                ...
                // сортировка тоже массив, то берем направления с тем же ключемs
                if (is_array($sort)) {
                    if (isset($sort[$key])) {
                        $sort = $sort[$key];
                    }
                ...
                $result = array_merge($result, [$order => $sort]);
            }
        }  else {
        ...
        if ($clear_columns && $result) {
            ...
            foreach ($result as $column => $value) {
                $new_result[explode('.', $column)[1]] = $value;
            }
            $result = $new_result;
        }
    ...
    {
        $hash_name = 'total_' . $this->getModel()::tableName();
